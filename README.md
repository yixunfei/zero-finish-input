...
| 安全剪贴板 | 默认关闭，支持选中文字或分享文本后认证导入，每次访问要求系统认证 |
| 系统剪贴板防护 | 默认关闭，可分别设置监听、提醒、清理方式和清理前身份认证，不读取正文 |
...
- 支持文字处理动作的应用中，选中文字后可在菜单或“更多”中选择“复制到 ZeroInput”；也可
...
- **监听系统剪贴板变化**：默认关闭。选用 ZeroInput 且输入法服务运行后，才尝试接收变化。
  启用或重新开始监听时只建立基线，不清理已存在的内容。关闭监听会使旧清理请求失效。
...
- **清理方式**：默认“不清理”，也可选择“用户确认后清理”或“自动清理”。自动清理启用前会
  提示普通复制粘贴可能失效、清理不可撤销，且不会自动保存内容。
- **清理前验证身份**：独立开关，默认关闭。开启后要求生物识别或设备凭据，认证通过后仍需
  点击“确认清空”。不能与后台自动清理并用；启用身份认证会将自动清理调整为确认清理。
- **检查并清理当前项**：开启监听设置并选用 ZeroInput 后，可主动检查已有的当前项，无需等待
...
厂商和其他输入法显示的“剪贴板历史”“收藏”不等同于 Android 当前项。它们不受上述清理接口
控制，需要到保存记录的应用中关闭历史功能并删除。选字菜单缺少“复制到 ZeroInput”时，可尝试
源应用的纯文本分享入口；如果源应用和厂商均不提供这些入口，普通应用无法强制注入选字菜单。
悬浮窗权限也不能取得任意页面的选中文字。
